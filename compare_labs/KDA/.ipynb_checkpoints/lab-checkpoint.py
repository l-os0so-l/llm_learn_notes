...
cfg = {
    ...
    "n_heads": 12,            # 注意力头数
    "n_layers": 8,            # 层数
    "chunk_size": 32,         # KDA 分块大小
    "context_size": 1024,     # 最大上下文长度
    "drop_rate": 0.1,         # Dropout 率
    "use_short_conv": True,   # 使用短卷积
    "conv_size": 4,           # 卷积核大小
    "max_new_tokens": 50,     # 生成 token 数
    "temperature": 0.8,       # 采样温度
    "topk": 40,               # Top-k 采样
    "grad_clip": 1.0,         # 梯度裁剪
    "check_numerics": True,   # 数值检查开关
    "use_amp": False,          # 自动混合精度
    "use_state": True,       # 生成时是否使用状态缓存（增量解码）
}
...
if __name__ == "__main__":
    # train数据集，618个batch， 1024长度（示例值，实际取决于文件大小）
    main(
        batch_size=16, 
        num_epochs=5, 
        lr=1e-4, 
