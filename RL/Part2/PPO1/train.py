...
class TrainConfig:
    ...
    output_dir: str = "Part2/PPO1/checkpoints"
    lr: float = 1e-5
    ...
    output_res_dir: str = "Part2/PPO1/res"
    # Online Filter 参数
...
def main(config: Optional[TrainConfig] = None):
    ...
    trainer = PPOVLMTrainer(
        ...
        var_lower=config.var_lower,
        var_upper=config.var_upper,
    )
    ...
    filter_tag = "filter" if config.use_filter else "nofilter"
    log_dir = os.path.join(config.output_res_dir, "logs", f"{run_timestamp}_{filter_tag}")
    os.makedirs(log_dir, exist_ok=True)
    log_file_path = os.path.join(log_dir, f"train_{run_timestamp}_{filter_tag}.log")
    metrics_file_path = os.path.join(log_dir, f"metrics_history_{run_timestamp}_{filter_tag}.json")
    ...
    for step in range(config.max_steps):
        ...
        if step % 3 == 0 and avg_stats:
            ...
            # 原始方差统计（跨所有 turn 的平均）
            raw_var_mean = avg_stats.get('raw_var_mean', 0)
            raw_var_max = avg_stats.get('raw_var_max', 0)
            raw_global_var = avg_stats.get('raw_global_var', 0)
            ...
            metrics_history["raw_var_mean"].append(raw_var_mean)
            metrics_history["raw_var_max"].append(raw_var_max)
            metrics_history["raw_global_var"].append(raw_global_var)
            ...
            _log_print(
                ...
                f"raw_var_mean={raw_var_mean:.4f} | "
                f"raw_var_max={raw_var_max:.4f} | "
                f"raw_gvar={raw_global_var:.4f}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_filter", action="store_true", default=TrainConfig.use_filter, help="是否使用 online variance filter")
    parser.add_argument("--var_lower", type=float, default=TrainConfig.var_lower, help="样本方差下限")
    parser.add_argument("--var_upper", type=float, default=TrainConfig.var_upper, help="样本方差上限")
