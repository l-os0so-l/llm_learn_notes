...
class PPOVLMTrainer:
    def __init__(
        ...
        var_lower: float = 0.01,
        var_upper: float = 10.0,
    ):
    ...
    def _compute_gae(
        ...
        Args:
            ...
            prompt_len:   int
            response_mask: [batch, response_len]  有效token mask（1=有效, 0=padding）

        Returns:
            advantages: [batch, response_len]
            returns:    [batch, response_len]
            var_stats:  dict，包含原始方差统计（用于日志）
        """
        ...
        with torch.no_grad():
            for k in range(batch_size):
                if response_mask is not None:
                    valid = advantages[k][response_mask[k] == 1]
                else:
                    valid = advantages[k]
                if valid.numel() > 1:
                    raw_var_per_sample.append(valid.var(unbiased=False).item())
                else:
                    raw_var_per_sample.append(0.0)

            if response_mask is not None:
                raw_global_var = advantages[response_mask == 1].var(unbiased=False).item()
            else:
                raw_global_var = advantages.var(unbiased=False).item()

        var_stats = {
            "raw_var_mean": sum(raw_var_per_sample) / len(raw_var_per_sample) if raw_var_per_sample else 0.0,
            "raw_var_max": max(raw_var_per_sample) if raw_var_per_sample else 0.0,
            "raw_var_min": min(raw_var_per_sample) if raw_var_per_sample else 0.0,
            "raw_global_var": raw_global_var,
        }

        # Online Filter：per-sample 方差控制
        if self.use_filter:
            mu = advantages.mean()
            ...
            for k in range(batch_size):
                ...
                if valid.numel() > 1:
                    v_k = valid.var(unbiased=False)
                    if v_k > self.var_upper:
                        r = (self.var_upper / v_k).sqrt()
                        advantages[k] = advantages[k] * r
                    elif v_k < self.var_lower:
                        r = (self.var_lower / v_k).sqrt()
                        advantages[k] = advantages[k] * r
        ...
        return advantages, returns, var_stats
    ...
    def ppo_step(
        ...
        advantages, returns, var_stats = self._compute_gae(
            rewards, old_logprobs, ref_logprobs, values, prompt_len, response_mask
        )
